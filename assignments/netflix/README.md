...
* Creates a new column `complete_director` with all the same values in the `director` column, excpet missing directors are replaced with the string "Unknown"
* Exports a new CSV file containing only the `title` and the `complete_director` columns, using `import csv` and `csv.writer`.
...
4. **Write the output CSV**
   ...
   * Include only these columns in the output (in this exact order):
